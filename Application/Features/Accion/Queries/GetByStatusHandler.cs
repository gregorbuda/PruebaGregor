...
{
    public class GetByStatusHandler : IRequestHandler<GetByStatus, ApiResponse<List<AccionTexto>>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetByStatusHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        public async Task<ApiResponse<List<AccionTexto>>> Handle(GetByStatus request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var list = await _unitOfWork.accionesRepository.GetByStatus(request._status);
