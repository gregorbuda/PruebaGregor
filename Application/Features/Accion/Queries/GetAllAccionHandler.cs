...
{
    public class GetAllAccionHandler : IRequestHandler<GetAllAccion, ApiResponse<List<AccionTexto>>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetAllAccionHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }
        public async Task<ApiResponse<List<AccionTexto>>> Handle(GetAllAccion request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var list = await _unitOfWork.accionesRepository.GetAllAsync();
